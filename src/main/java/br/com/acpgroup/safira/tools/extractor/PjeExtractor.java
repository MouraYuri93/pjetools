...
public class PjeExtractor {
    ...
    public void run() {
        ...
        while (execute) {
            System.out.println("Navegue até a página de captura de avisos que deseja extrair");
            awaitManualContinue();
            ...
            for (WebElement tr : trs) {
                ...
                if (matcherTipoDocumento.find()) {
                    ...
                    log.warn("tipoDocumento: " + tipoDocumento + " não pode ser carregado");
                }
                ...
                if (matcherProcesso.find()) {
                    ...
                    log.warn("Processo: " + processo + " não pode ser carregado");
                }
        ...
        dbTemp.forEach((fileName, data) -> {
            try {
                excelHandler.exportToExcel(data, fileName);
            } catch (IOException e) {
                log.error("Erro ao exportar dados para o arquivo Excel: " + e.getMessage());
            }
        });
    }
    ...
    private Date obterDataAfter(String value) {
        ...
        if (matcherData.find()) {
            ...
            log.warn("Data : " + value + " não pode ser carregada");
            return null;
        }
    }
}